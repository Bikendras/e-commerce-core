...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
      cb(null, 'E:/bikendra/Bikendra/eCommerce/e-commerce-dev-ui/public') // destination is a place where it store the image...(jaha par image ko store ko store karna hai waha ka Adrress dena hai..)
    },
    filename: function (req, file, cb) {
      cb(null,`${Date.now()}-${file.originalname}`) // backend me us file ka original name jo user ne jiss name se save kiya tha usi name se save kiya ja raha hai...
    }
  })
...
router.get("/getdata",verify,indexController.alluserApi);
...
router.post("/userImageUpload/:email", upload?.single("image"),indexController.imageUpload );// orderBooked
router.get("/profile/:email",upload.single(), indexController.getImageApi);
...
router.get("/pdfDownload",indexController.generatePdf) // Download pdf
